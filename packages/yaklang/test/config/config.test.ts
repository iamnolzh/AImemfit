...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "yaklang.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .yaklang directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const opencodeDir = path.join(dir, ".yaklang")
...
test("resolves scoped npm plugins in config", async () => {
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "yaklang.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .yaklang config
      const projectDir = path.join(dir, "project")
      const opencodeDir = path.join(projectDir, ".yaklang")
      await fs.mkdir(opencodeDir, { recursive: true })
      ...
      // Local .yaklang config with different plugins
      await Bun.write(
        path.join(opencodeDir, "yaklang.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .yaklang config with some overlapping plugins
      await Bun.write(
        path.join(opencodeDir, "yaklang.json"),
        JSON.stringify({
          $schema: "https://opencode.ai/config.json",
