...
export const UPDATER_ENABLED = window.__YAKLANG__?.updaterEnabled ?? false
...
export async function runUpdater({ alertOnFail }: { alertOnFail: boolean }) {
  ...
  if (!update) {
    if (alertOnFail)
      await message("You are already using the latest version of Yaklang", { title: "No Update Available" })
    return
  }
  ...
  const shouldUpdate = await ask(
    `Version ${update.version} of Yaklang has been downloaded, would you like to install it and relaunch?`,
