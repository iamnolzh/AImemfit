...
function formatInitError(error: InitError): string {
  ...
  switch (error.name) {
    case "MCPFailed":
      return `MCP server "${data.name}" failed. Note, yaklang does not support MCP authentication yet.`
    ...
    case "ProviderModelNotFoundError": {
      ...
      return [
        ...
        `Check your config (yaklang.json) provider/model names`,
      ].join("\n")
    }
    ...
    <div class="relative flex-1 h-screen w-screen min-h-0 flex flex-col items-center justify-center bg-background-base font-sans">
      <div class="w-2/3 max-w-3xl flex flex-col items-center justify-center gap-8">
        ...
        <div class="flex flex-col items-center gap-2 text-center">
          <h1 class="text-lg font-medium text-text-strong">出错了</h1>
          <p class="text-sm text-text-weak">加载应用时发生错误。</p>
        </div>
        <TextField
        ...
        <div class="flex flex-col items-center gap-2">
          <div class="flex items-center justify-center gap-1">
            ...
            <button
              ...
              onClick={() => platform.openLink("https://yaklang.com/desktop-feedback")}
            >
              <div>（Discord）</div>
