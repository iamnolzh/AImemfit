...
export default function Layout(props: ParentProps) {
  ...
  onMount(() => {
    ...
    const unsub = globalSDK.event.listen((e) => {
      ...
      const description = `${projectName} 中的「${sessionTitle}」需要权限`
  ...
  command.register(() => {
    ...
    for (const scheme of colorSchemeOrder) {
      commands.push({
        ...
        title: `使用配色：${colorSchemeLabel[scheme]}`,
        category: "主题",
        onSelect: () => theme.commitPreview(),
        onHighlight: () => {
            ...
            >
              ...
              <Show when={props.session.summary?.files}>
                <div class="flex justify-between items-center self-stretch">
                  <span class="text-12-regular text-text-weak">{`${props.session.summary?.files ?? 0} 个文件已变更`}</span>
      ...
      <div class="flex flex-col self-stretch h-full items-center justify-between overflow-hidden min-h-0">
        ...
        <div class="flex flex-col gap-1.5 self-stretch items-start shrink-0 px-2 py-3">
            ...
            <Match when={providers.all().length > 0}>
              <Tooltip placement="right" value="连接模型提供商" inactive={expanded()}>
                <Button
                  ...
                  onClick={connectProvider}
                >
                  <Show when={expanded()}>连接模型提供商</Show>
                </Button>
              </Tooltip>
            </Match>
          </Switch>
          ...
          {/* <Tooltip placement="right" value="Share feedback" inactive={expanded()}>
            <Button
              as={"a"}
              href="https://yaklang.com/desktop-feedback"
              target="_blank"
              class="flex w-full text-left justify-start text-text-base stroke-[1.5px] rounded-lg px-2"
              variant="ghost"
              size="large"
