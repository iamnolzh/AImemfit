...
import { SKILL_PROTECTED_MESSAGE } from "../../src/tool/skill-protected"
...
describe("tool.patch", () => {
  test("should reject patch that adds file under .opencode/skill (skill protected)", async () => {
    await using fixture = await tmpdir()
    await Instance.provide({
      directory: fixture.path,
      fn: async () => {
        const patchText = `*** Begin Patch
*** Add File: .opencode/skill/foo/SKILL.md
+# fake skill
+content
*** End Patch`
        await expect(patchTool.execute({ patchText }, ctx)).rejects.toThrow(SKILL_PROTECTED_MESSAGE)
      },
    })
  })
